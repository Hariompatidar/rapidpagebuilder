...
const AllPages = () => {
  ...
  useEffect(() => {
    const debounced = debounce(() => {
      dispatch(getAllPages({ search: debouncedSearchTerm, ...filters }));
    }, 500);
          ...
          <table className="min-w-full ">
            ...
            <tbody>
              {allPages.map((page, index) => (
                <tr key={page?.id} className="border-b hover:bg-[#f3f3f3] group/item">
                  <td className="table-data flex gap-2 justify-between items-center pr-2 ">
                    {page?.title}{" "}
