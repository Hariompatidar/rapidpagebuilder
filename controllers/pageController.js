  ...
  await Promise.all(
    files.map(async (file) => {
      const result = await uploadPdfToCloudinary(
        file,
        process.env.CLOUDINARY_DOCUMENT_FOLDER_NAME
      );
      attachments.push({
...
exports.updatePage = catchAsyncErrors(async (req, res, next) => {
  ...
  if (page.title !== title) {
    page.title = title;
    page.titleSlug = slugify(title, { lower: true });
  }
  ...
  for (const existingAttachment of page.attachments) {
    const isIncoming = incomingAttachmentsIds.includes(
      existingAttachment._id.toString()
    );
...
exports.deletePage = catchAsyncErrors(async (req, res, next) => {
  ...
  const scheduledPublication= await SchedulePublication.findOne({page:page._id});
  if(scheduledPublication){
    scheduledPublication.status= "Deleted";
...
exports.getPageByIdOrSlug = catchAsyncErrors(async (req, res, next) => {
  ...
  if (mongoose.Types.ObjectId.isValid(id)) {
    ...
    page = await Page.findOne({ slug: id, isDeleted: false }).populate(
      "createdBy"
    );
  }
...
exports.getAllPages = catchAsyncErrors(async (req, res, next) => {
  ...
  if (search) {
    const searchTerm = slugify(search, { lower: true });
    query.titleSlug = { $regex: searchTerm, $options: "i" };
  }
